...
- For any two events the probability that at least one occurs is the sum of their
    probabilities minus their intersection. i.e., can't just sum probabilities
...
- A **random variable** is a quantity whose value changes, and is the numerical
  outcome of an experiment.

- Can be either discrete or continuous
- Discrete random variables take on only a countable number of possibilities and
  we talk about the probability that they take specific values. We obtain the
  value of discrete variables by counting.
- Continuous random variables can conceptually take any value on the real line
  or some subset of the real line and we talk about the probability that they
  lie within some range. The above makes no sense if you don't explain
  what a real line is. We obtain continuous variables by measuring, e.g.,
...
- The web site traffic on a given day (discrete, number of web hits)
- The BMI of a subject four years after a baseline measurement (continuous, measurement)
- The hypertension status of a subject randomly drawn from a population (1 or 0
  for yes or no, so discrete)
- The number of people who click on an ad (discrete, number of web hits) 
- Intelligence quotients for a sample of children (continuous)

---

## PMF for discrete vars
...
## PDF for continuous random vars
...
# pbeta = probability of beta density
pbeta(0.75, 2, 1)
```
...
This is what pbeta is doing, returning the CDF.
...
If you were the 95th percentile on an exam, 95% of people scored lower than you
and 5% scored higher. These are sample quantities. Here we define their
...
- Therefore, about 0.7071 or 70% of calls being answered on a random day is the median.
...
- We're referring to **population quantities**. Therefore, the median being
