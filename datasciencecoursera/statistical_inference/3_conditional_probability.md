...
- P(A n B) is the probability of the intersection of A and B

- Notice that if $A$ and $B$ are independent (defined later in the lecture), then
  $$
  P(A ~|~ B) = \frac{P(A) P(B)}{P(B)} = P(A)
  $$
i.e., if they are independent, then the probability of A occurring has nothing
to do with whether B occurred or not
...
e.g., if we know the probability of B given A, we can figure out the probability
of A given B - we just need to know the probability of B marginalised over A.
...
- Suppose that a subject, from a population with a .1% prevalence of HIV,
  receives a positive test result. What is the positive predictive value? i.e.
  what's the chance they actually have HIV.
...
the diagnostic likelihood ratio of a positive test result is the factor by which you
multiply your odds in the presence of a positive test to obtain the post-test odds
